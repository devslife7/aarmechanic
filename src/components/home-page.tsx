...
type HomePageProps = {
  lang: Locale;
  initialReviews?: LiveReviews | null;
};

export function HomePage({ lang, initialReviews = null }: HomePageProps) {
  ...
  const [liveReviews, setLiveReviews] = useState<LiveReviews | null>(initialReviews);
  ...
  return (
    <div className="bg-midnight text-white font-sans leading-relaxed antialiased overflow-x-hidden min-h-screen">
      ...
      {/* ── NAV ─────────────────────────────────────── */}
      <nav className={`fixed top-0 left-0 right-0 z-[100] px-10 max-sm:px-5 bg-white border-b border-gray-200 transition-all duration-300 ${scrolled ? "backdrop-blur-xl" : ""}`} aria-label="Primary">
        <div className="max-w-[1280px] mx-auto flex items-center justify-between h-[90px]">
          <div className="flex items-center gap-8 max-sm:gap-3">
            <Link href={home} className="flex items-center no-underline group" aria-label="Anywhere Auto Repair — Home">
              <img src="/logo.png" alt="Anywhere Auto Repair — mobile mechanic serving DC, MD, and VA" width={110} height={110} className="w-[110px] h-[110px] rounded-full object-cover border-2 border-gray-200 transition-colors duration-300 group-hover:border-blue-500 translate-y-3" />
            </Link>
            <div className="flex lg:hidden items-center bg-gray-100 rounded-full p-[3px] border border-gray-200">
              <Link prefetch={false} href="/en" className={`px-3 py-[5px] rounded-full text-[0.72rem] font-bold transition-all duration-300 no-underline ${lang === "en" ? "bg-blue-500 text-white" : "bg-transparent text-gray-500"}`}>EN</Link>
              <Link prefetch={false} href="/es" className={`px-3 py-[5px] rounded-full text-[0.72rem] font-bold transition-all duration-300 no-underline ${lang === "es" ? "bg-blue-500 text-white" : "bg-transparent text-gray-500"}`}>ES</Link>
            </div>
        ...
        <div className="max-w-[1280px] mx-auto w-full grid grid-cols-[1.1fr_0.9fr] gap-20 items-center max-lg:grid-cols-1 max-lg:gap-12">
          <div>
            ...
            <div className="flex gap-4 items-center flex-wrap reveal reveal-delay-3">
              <a href={contactHref} className="inline-flex items-center gap-2.5 bg-blue-500 text-white px-9 py-[18px] rounded-full font-semibold text-[0.95rem] no-underline transition-all duration-300 hover:bg-blue-400 hover:-translate-y-0.5 hover:shadow-[0_12px_40px_rgba(59,130,246,0.3)]">
                <WhatsAppIcon />
      ...
      {/* ── THE PROCESS ─────────────────────────────── */}
      <section className="py-16 px-10 max-sm:py-12 max-sm:px-5 relative z-[1] bg-white scroll-mt-[90px]" id="how" aria-labelledby="how-heading">
        <div className="max-w-[1100px] mx-auto">
          <div className="text-center mb-12 max-sm:mb-10 reveal">
            <h2 id="how-heading" className="font-serif text-[3.5rem] max-sm:text-[2.5rem] font-normal leading-[1.1] tracking-[-0.01em] text-[#0f1b3d]">
          ...
          <div className="grid grid-cols-3 max-sm:grid-cols-1 gap-8 max-sm:gap-14">
            {tx.process.steps.map((step, i) => (
              <div key={i} className={`text-center reveal${i > 0 ? ` reveal-delay-${i}` : ""}`}>
                ...
                <h3 className="font-serif text-[1.3rem] font-normal text-[#0f1b3d] mb-4 tracking-normal">{step.title}</h3>
                <p className="text-[#5a6275] text-[0.95rem] leading-[1.8] font-light max-w-[300px] mx-auto">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── SERVICES ────────────────────────────────── */}
      <section className="py-[100px] px-10 max-sm:py-16 max-sm:px-5 relative z-[1] bg-midnight scroll-mt-[90px]" id="services" aria-labelledby="services-heading">
        <div className="max-w-[1280px] mx-auto">
          <div className="mb-16 max-sm:mb-12 reveal">
            ...
            <h2 id="services-heading" className="font-serif text-[3rem] max-sm:text-[2rem] font-normal leading-[1.1] tracking-[-0.02em] text-white">
          ...
          <div className="grid grid-cols-[1fr_1.2fr] max-lg:grid-cols-1 gap-10 items-start">
            <div className="flex flex-col gap-px rounded-2xl overflow-hidden reveal">
              {tx.services.items.map((svc, i) => (
                <Link key={i} href={`${home}/services/${SERVICE_SLUGS[i]}`} className="group p-6 bg-white/[0.03] hover:bg-white/[0.07] transition-all duration-300 no-underline block">
            ...
            <div className="relative rounded-2xl overflow-hidden max-lg:order-first reveal reveal-delay-1">
              <img src={serviceImages[3]} alt={lang === "es" ? "Mecánico móvil trabajando en un vehículo" : "Mobile mechanic at work on a vehicle"} className="w-full h-full min-h-[520px] max-lg:min-h-[300px] object-cover" />
      ...
      {/* ── BRANDS MARQUEE ──────────────────────────── */}
      <section className="bg-slate-50 overflow-hidden relative z-[2] py-20 max-sm:py-12 border-y border-gray-200" aria-labelledby="brands-heading">
        <div className="max-w-[1280px] mx-auto px-10 max-sm:px-5">
          <div className="text-center mb-12 reveal">
            ...
            <h2 id="brands-heading" className="font-serif text-[2.8rem] max-sm:text-[1.9rem] font-normal leading-[1.1] tracking-[-0.02em] text-midnight">{tx.brands.title}</h2>
          </div>
        </div>
          ...
          <div className="flex w-max animate-marquee items-center brands-track">
            {[...brandsList, ...brandsList].map((brand, i) => (
              <div key={i} className="shrink-0 flex items-center justify-center mx-10 max-sm:mx-6">
                <img src={brand.logo} alt={`${brand.name} logo — we service ${brand.name} vehicles`} className="h-[50px] max-sm:h-[36px] w-[110px] max-sm:w-[80px] object-contain" />
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── SERVICE AREA ────────────────────────────── */}
      <section className="relative z-[1] bg-midnight border-t border-white/8 scroll-mt-[90px]" id="area" aria-labelledby="area-heading">
        <div className="max-w-[1280px] mx-auto px-10 max-sm:px-5 py-[100px] max-sm:py-16">
          <div className="mb-12 reveal">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <h2 id="area-heading" className="font-serif text-[3rem] max-sm:text-[2.2rem] font-normal leading-[1.1] tracking-[-0.02em] text-white">
              ...
              {tx.coverage.areas.map((area, i) => {
                ...
                return (
                  <div key={i}
                    ...
                    <div>
                      <h3 className="font-serif text-[1rem] font-normal text-white mb-1">{area.name}</h3>
          ...
          <div className="grid grid-cols-[0.85fr_1.15fr] max-lg:grid-cols-1 gap-20 max-lg:gap-12 items-center">
            <div className="relative reveal">
              <div className="rounded-[28px] h-[500px] max-sm:h-[360px] relative overflow-hidden border border-white/12">
                <img src="/tyler.png" alt="Tyler, the owner and mobile mechanic behind Anywhere Auto Repair" className="w-full h-full object-cover object-top" />
            ...
            <div>
              ...
              <h2 id="about-heading" className="font-serif text-[2.8rem] max-sm:text-[2.2rem] font-normal tracking-[-0.02em] mb-6 leading-[1.15] reveal reveal-delay-1">
      ...
      {/* ── GOOGLE REVIEWS ───────────────────────────── */}
      <section className="py-[120px] px-10 max-sm:py-20 max-sm:px-5 relative z-[1] bg-white scroll-mt-[90px]" id="reviews" aria-labelledby="reviews-heading">
        <div className="max-w-[1280px] mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-10 reveal reveal-delay-1">
            <h2 id="reviews-heading" className="font-serif text-[3rem] max-sm:text-[2rem] font-normal leading-[1.1] tracking-[-0.02em] text-gray-900">
          ...
          <div ref={reviewsRef} className="grid grid-cols-3 max-lg:grid-cols-2 max-sm:grid-cols-1 gap-5 reveal reveal-delay-2">
      ...
      {/* ── CONTACT ─────────────────────────────────── */}
      <section className="bg-deep border-t border-white/12 py-[120px] px-10 max-sm:py-20 max-sm:px-5 relative z-[1] scroll-mt-[90px]" id="contact" aria-labelledby="contact-heading">
        <div className="max-w-[1280px] mx-auto grid grid-cols-2 max-lg:grid-cols-1 gap-20 max-lg:gap-12 items-start">
          <div>
            ...
            <h2 id="contact-heading" className="font-serif text-[3rem] max-sm:text-[2rem] font-normal leading-[1.1] tracking-[-0.02em] mb-4 reveal reveal-delay-1">
          ...
          <div className="relative overflow-hidden rounded-3xl p-11 max-sm:p-7 border border-white/12 reveal reveal-delay-2" style={{ background: "linear-gradient(170deg,#151d35,#0f1629)" }}>
            ...
            {formSubmitted ? (
              <div className="text-center py-10">
                ...
                <h3 className="font-serif text-[1.6rem] mb-2">{tx.contact.successTitle}</h3>
        ...
        <div className="max-w-[1280px] mx-auto px-10 max-sm:px-5 py-14 grid grid-cols-[1.6fr_1fr_1fr_1fr] max-lg:grid-cols-2 max-sm:grid-cols-1 gap-12 border-b border-white/[0.05]">
          <div className="max-lg:col-span-2 max-sm:col-span-1">
            <div className="flex items-center gap-3 mb-5">
              <img src="/logo.png" alt="Anywhere Auto Repair logo" className="w-12 h-12 rounded-2xl opacity-70 flex-shrink-0" />
              <div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {tx.services.items.map((s, i) => (
                <li key={i}><Link href={`${home}/services/${SERVICE_SLUGS[i]}`} className="text-[0.85rem] text-white/50 hover:text-white transition-colors no-underline">{s.name}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {AREA_SLUGS.map((slug) => {
                ...
                return (
                  <li key={slug}>
                    <Link href={`${home}/service-area/${slug}`} className="text-[0.85rem] text-white/50 hover:text-white transition-colors no-underline">{area.name}</Link>
                  </li>
                );
              })}
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-3 mb-8">
              <li><a href={`${home}#how`} className="text-[0.85rem] text-white/50 hover:text-white transition-colors no-underline">{tx.nav.how}</a></li>
              <li><a href={`${home}#about`} className="text-[0.85rem] text-white/50 hover:text-white transition-colors no-underline">{tx.nav.about}</a></li>
              <li><a href={`${home}#reviews`} className="text-[0.85rem] text-white/50 hover:text-white transition-colors no-underline">{tx.nav.reviews}</a></li>
              <li><Link href={`${home}/faq`} className="text-[0.85rem] text-white/50 hover:text-white transition-colors no-underline">{tx.nav.faq}</Link></li>
              <li><Link href={`${home}/blog`} className="text-[0.85rem] text-white/50 hover:text-white transition-colors no-underline">{tx.nav.blog}</Link></li>
              <li><Link href={`/${otherLang}`} hrefLang={otherLang === "en" ? "en-US" : "es-US"} className="text-[0.85rem] text-white/50 hover:text-white transition-colors no-underline">{otherLang === "en" ? "English" : "Español"}</Link></li>
            </ul>
